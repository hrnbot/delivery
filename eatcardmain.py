...
class Driver:
    ...
    def serve_order(self):
        ...
        write_in_separate_file(str(self.orders[0].id),str(self.id) + " Driver serving Order" + str(self.orders[0].id))
    ...
    def order_pickup(self):
        ...
        self.target_location = self.orders[0].delivery_location
        ...
        for order in orders:
            if order.pickup_time - datetime_to_seconds(datetime.datetime.now()) > check_order_thresold:
                ...
                if len(drivers_of_restaurant) > 0:
                    ...
                    # print(driver_first)
                    print("First Driver Reach to pickup time ", str(datetime_from_timestamp(
